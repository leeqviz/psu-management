...
import { numCase } from "./stringMapper";
...
export const getLocaleDate = (date: string = "01.01.1970") => {
  ...
  if(date.split(".").length === 3)
    return new Date(`${dateParts[2]}-${dateParts[1]}-${dateParts[0]}`).toLocaleDateString();
  else
...
export const getISODateFromLocaleDate = (
  ...
  if(dateParts && date?.split(".").length === 3)
    return getISODate(new Date(`${dateParts[2]}-${dateParts[1]}-${dateParts[0]}`), isShort);
  else {
